...
from app.core.schedule_poller import run_schedule_poller
...
FLUTTER_DEP_UPDATER_DEFINITION = {
...
def _seed_default_template() -> None:
    ...
    try:
        ...
        existing_flutter = db.query(PipelineTemplate).filter_by(name="Flutter Dependency Updater").first()
        if not existing_flutter:
            flutter_template = PipelineTemplate(
                name="Flutter Dependency Updater",
                description="Audit and update all Flutter dependencies to latest major versions, one by one. Fixes or reverts broken updates, creates PR with report.",
                definition=FLUTTER_DEP_UPDATER_DEFINITION,
            )
            db.add(flutter_template)
            db.commit()
            logger.info("Seeded Flutter Dependency Updater template")
    finally:
        db.close()
...
async def lifespan(app: FastAPI):
    ...
    sse_task = asyncio.create_task(_session_event_consumer())
    poller_task = asyncio.create_task(run_schedule_poller())
    yield
    poller_task.cancel()
    sse_task.cancel()
    for t in (poller_task, sse_task):
        try:
            await t
